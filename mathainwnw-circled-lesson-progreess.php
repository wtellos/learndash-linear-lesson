...
function cardet_custom_breadcrumbs(){
   ...
   $site_home_text = cardet_home_breadcrumbs_string();
    ...
    //    echo "<a href='$site_url'>$site_home_text</a><span class='cardet-breadcrumbs-divider'>/</span><span class=cardet-span-text-breadcrumbs>$course_title</span>";
       echo "<a class='el-link uk-button uk-button-primary' href='$site_url'>Back</a>";
    }
    ...
    //    echo "<a class='el-link uk-button uk-button-primary' href='$course_link'><span>Back to</span> $course_title </a>";
       echo "<a class='el-link uk-button uk-button-primary' href='$course_link'><span>Back</span></a>";
    }
...
function cardet_display_courses(){
    ob_start();
        ...
        echo '<div class="uk-child-width-1-2 uk-flex uk-flex-center" uk-grid uk-scrollspy="target: > a; cls: uk-animation-fade; delay: 300">';
        ...
        if (is_user_logged_in()) {
          ...
          $course_link = get_the_permalink();
            ...
            echo "<div><a class='cardet-course-grid' href='" . $course_link . "'><div class='cardet-course-grid-image uk-text-center'><img src='". get_the_post_thumbnail_url() ."'></div><div class='uk-text-center cardet-course-header'>
            </div></a></div>";
            //        <h3 class='uk-heading-small uk-font-secondary uk-margin-top uk-margin-remove-bottom'>" . get_the_title(). "</h3>
        endwhile;
...
/////////////////////// FORMINATOR Form submission check and Shortcode ///////////////////////
    ...
    //         }
    //     } else {
    //         return '<p class="lead">You need to log in to access the Games</p>';
    //     }
    // }
...
function multi_lesson_function($course_id) {
    //Display lessons function
        ...
        $form_result = 1; // Altered value for App.
        $lesson_link = get_post_permalink($lesson->ID);
        
        foreach ($lessons as $index => $lesson){
            $lesson_id = $lesson->ID;
            $lesson_image = get_the_post_thumbnail($lesson_id); 
            $lesson_title = $lesson->post_title;
            $isComplete = learndash_is_lesson_complete($user_id, $lesson_id);
            $lesson_link = "#0";
            $lesson_not_available = '';

            // NEW: Calculate per-lesson topic completion
            ...
            $completed_topics_percentage = $total_topics > 0 ? round(($completed_topics / $total_topics) * 100) : ($isComplete ? 100 : 0);

            // Linear logic
            if ($isFirstLesson == $lesson_id && $form_result == 1) {
                $lesson_link = get_post_permalink($lesson_id);
            } else if ($index > 0) {
            ...
            $lesson_completion_class = $isComplete ? 'cardet-lesson-completed' : 'cardet-lesson-not-completed';


$radius = 28;
$circumference = 2 * pi() * $radius;
$progress = max(0, min(100, $completed_topics_percentage)); // clamp 0–100
...
<div class='th--child'>
    <a class='cardet-lesson-grid uk-position-relative uk-animation-fade {$lesson_completion_class} {$lesson_not_available}' href='{$lesson_link}'>
        ...
        <div class='cardet-lesson-grid-image uk-text-center uk-padding-small'>
            ...
            <div class='svg-progress-wrapper'>
                <svg class='progress-ring' width='80' height='80'>
                    <circle class='progress-ring-bg' cx='40' cy='40' r='{$radius}' stroke-width='6' fill='transparent' />
                    <circle class='progress-ring-circle' cx='40' cy='40' r='{$radius}' stroke-width='6' fill='transparent'
                        stroke-dasharray='{$circumference}'
                        stroke-dashoffset='{$offset}' />
                </svg>
                <div class='progress-ring-center'>
                    " . ($lesson_not_available ? 
                        "<img src='https://app.mathainwellinika.com/wp-content/uploads/2025/09/locked.svg'>" : 
                        ($isComplete ?
                            "<img src='https://app.mathainwellinika.com/wp-content/uploads/2025/09/completed.svg'>" :
        ...
        <div class='uk-text-center DDuk-text-left cardet-lesson-header'>
            <h2 class='uk-heading-small uk-margin-remove'>{$lesson_title}</h2>
        </div>
    </a>
</div>";
...
// function only_email_registration_form() {

//     ob_start();
//     $mathainw_username = "";
//     $mathainw_user_email = sanitize_email($_POST['cprf_user_login']);
//     $default_mathainw_password = "mathainw2025";


   // ? >
// <!--     <form method="post">
//         <label class="" for="cprf_user_login">Your Email:</label>
//         <input class="uk-input uk-form-width-medium"  type="email" name="cprf_user_login" id="cprf_user_login" required><br><br>
//         <button class="uk-button uk-button-primary" type="submit" name="cprf_reset_password" value="" onclick="createUser_and_sendEmail()">Register</button>
//     </form> -->
   // <?php
...
// }
// add_shortcode('only_email_form', 'only_email_registration_form');
